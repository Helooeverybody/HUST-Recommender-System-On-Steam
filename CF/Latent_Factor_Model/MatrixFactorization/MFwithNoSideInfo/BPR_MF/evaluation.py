...
def recall_at_k(r, k, all_positives):
    r = np.asarray(r)[:k]
    return np.sum(r) / len(all_positives)
...
def compute_metrics(model, test_data, k=5):
    ...
    for u in test_data:
        ...
        ranked_items = [x for _, x in sorted(zip(scores, labels), reverse=True)]
        ndcg_score = ndcg_at_k(ranked_items, k)
        f1_score = f1_at_k(ranked_items, 20, u_likes)
...
def evaluate(model, test_loader, test_data, k=5):
    ...
    print(f'Accuracy: {accuracy:.4f} - NDCG@{k}: {ndcg_k:.4f} - F1@20: {f1_k:.4f}')